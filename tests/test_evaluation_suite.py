...
class TestEvaluationSuiteConfig(TestCase):
    def setUp(self):
        # Check that the EvaluationSuite loads successfully
        self.evaluation_suite = EvaluationSuite.load("tests/evaluation_suite_glue_ci.py")
    ...
    def test_running_evaluation_suite(self):
        ...
        result_values = [1.0, 0.5]

        # Check that the results are correct
        for r, value in zip(results, result_values):
            self.assertEqual(r["accuracy"], value)
