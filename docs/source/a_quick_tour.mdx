...
Some metrics can be configured with additional settings. E.g. `accuracy` has the extra setting `normalize` which defaults to `True`. To change it you have two options: pass it as a keyword argument with `load` or during `compute`. With `load` the setting is changed permanently for the module while passing it to `compute` only changes it for the duration of the `compute` call.
