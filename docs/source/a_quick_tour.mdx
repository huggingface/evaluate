...
🤗 Evaluate provides access to a wide range of evaluation tools. You can load metrics associated with benchmark datasets like GLUE or SQuAD, and complex metrics like BLEURT or BERTScore, with a single command: [`evaluate.load`]. Once you've loaded a metric, easily compute and evaluate a model's performance.
## ELI5: `evaluate.load`

When you call [`evaluate.load`], the metric loading script is downloaded and imported from GitHub (if it hasn't already been downloaded before). It contains information about the metric such as it's citation, homepage, and description.

The metric loading script will instantiate and return a `EvaluationModule` object. This stores the predictions and references, which you need to compute the metric values. The `EvaluationModule` object is stored as an Apache Arrow table. As a result, the predictions and references are stored directly on disk with memory-mapping. This enables 🤗 Evaluate to do a lazy computation of the metric, and makes it easier to gather all the predictions in a distributed setting.
