...
The metrics in `evaluate` can be easily integrated in the [`~transformers.Trainer`]. The `Trainer` accepts a `compute_metrics` keyword argument where a function to compute metrics can be passed. One can specify the evaluation interval with `evaluation_strategy` in the [`~transformers.TrainerArguments`] and based on that the model is applied to the evaluation said and the predictions and labels passed to `compute_metrics`.
