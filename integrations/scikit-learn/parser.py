...
for task_list, task_features in zip([classification_metrics, regression_metrics], [classification_features, regression_features]):
    for sk_metric in [func for func in getmembers(metrics, isfunction) if func[0] in task_list]:
        ...
        cookiecutter(
            "integrations/scikit-learn/template",
            no_input=True,
            extra_context=args,
            output_dir="sklearn-metrics",
...
subprocess.Popen(f"black --line-length 119 --target-version py36 sklearn-metrics".split())
subprocess.Popen(f"isort sklearn-metrics".split())